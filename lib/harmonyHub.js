...
const HarmonyHubClient = require("harmonyhubjs-client");
...
module.exports = class HarmonyHub {
  ...
  connect() {
    ...
    this.discovery.on("offline", hub => this.onHubOnline(hub));
  ...
  async onHubOnline(hub) {
    ...
    hub.client = await HarmonyHubClient(hub.ip);
    hub.commands = await hub.client.getAvailableCommands();
  ...
  sendCommand(hub, deviceHint, actionHint) {
    ...
    return hub.client.send("holdAction", "action=" + encodedAction + ":status=press")
  }
};
