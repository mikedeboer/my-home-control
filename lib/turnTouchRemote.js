    ...
    switch (event.button) {
      ...
      case "south":
        if (event.hold) {
          ...
          console.log("Already watching TV, channel DOWN.");
          await this.harmonyRemote.sendCommand(hub, "pvr", "channelDown");
        }
        break;
      case "west":
        if (isCurrentActivity(ACT_WATCHTV)) {
          console.log("Already watching TV, volume DOWN.");
